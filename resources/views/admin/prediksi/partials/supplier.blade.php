            ...
            <div style="background: #fafcff; border: 1px solid #e8eef8; border-radius: 12px; padding: 16px;">
                <div
                    ...
                    <div style="display: flex; align-items: center; gap: 8px;">
                        ...
                        <span class="mrp-required-amount" data-key="{{ $key }}" data-base-jumlah="{{ $val['jumlah'] }}" data-satuan="{{ $val['satuan'] }}" style="font-size: 0.8rem; font-weight: 700; color: #1a2b4a;">
                            Dibutuhkan: {{ number_format($val['jumlah'], 0, ',', '.') }} {{ $val['satuan'] }}
                        </span>
                    </div>
                </div>
                                ...
                                <div style="display: flex; gap: 8px; flex-wrap: wrap;">
                                    @if($sup->no_whatsapp)
                                        <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $sup->no_whatsapp) }}?text=Halo%20{{ urlencode($sup->nama_supplier) }},%20kami%20tertarik%20untuk%20memesan%20bahan%20baku%20{{ urlencode(strtolower($val['nama'])) }}%20sebanyak%20{{ $val['jumlah'] }}%20{{ $val['satuan'] }}."
                                            target="_blank"
                                            class="sup-wa-btn"
                                            data-sup-name="{{ $sup->nama_supplier }}"
                                            data-phone="{{ preg_replace('/[^0-9]/', '', $sup->no_whatsapp) }}"
                                            data-bahan-name="{{ strtolower($val['nama']) }}"
                                            ...
                                            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                                stroke-width="2.5">
                                                <path
                                                    d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1 2.2 1.72z" />
                                            </svg>
                                            WhatsApp
                                        </a>
                                    @endif
