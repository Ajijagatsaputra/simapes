        ...
        <div class="card">
            <div class="table-toolbar">
                ...
                <div class="toolbar-right">
                    <button type="button" class="btn-bulk-hapus" id="btnBulkDelete" onclick="submitBulkDelete()"
                        style="display: none;">
            ...
            <form id="formBulkDelete" method="POST" action="{{ route('admin.pelanggan.bulkDestroy') }}"
                style="display:none;">
                @csrf
                @method('DELETE')
                ...
                <thead>
                    <tr>
                        <th style="width:40px; text-align: center;">
                            <input type="checkbox" id="selectAllPelanggan" onchange="toggleSelectAll(this)"
                    ...
                    @forelse($pelanggan as $index => $p)
                        <tr
                            ...
                            <td style="text-align: center; vertical-align: middle;">
                                <input type="checkbox" class="select-pelanggan" value="{{ $p->id }}"
                                    onchange="updateBulkDeleteButton()" style="cursor: pointer; transform: scale(1.2);">
                            </td>
                            <td class="row-number">{{ $pelanggan->firstItem() + $index }}</td>
                            <td>
                                <div>
                                    <div style="font-weight: 700; color: #1a2b4a;">{{ $p->name }}</div>
                                    <div style="font-size: 0.72rem; color: #8ca0bf;">{{ $p->email }}</div>
                                </div>
                            </td>
                            <td>{{ $p->no_whatsapp ?? '-' }}</td>
                            <td>{{ $p->alamat ?? '-' }}</td>
                            ...
                            <td>
                                <div class="aksi-wrap">
                                    <form method="POST" action="{{ route('admin.pelanggan.destroy', $p->id) }}"
                                        id="form-hapus-pelanggan-{{ $p->id }}" style="display:inline">
                                        @csrf @method('DELETE')
                                        <button type="button" class="btn-hapus" title="Hapus" data-nama="{{ $p->name }}"
...
@push('scripts')
    <script>
        ...
        function toggleSelectAll(masterCheckbox) {
            const checkboxes = document.querySelectorAll('.select-pelanggan');
        ...
        function updateBulkDeleteButton() {
            const checkedCheckboxes = document.querySelectorAll('.select-pelanggan:checked');
            const totalCheckboxes = document.querySelectorAll('.select-pelanggan');
            const selectAllCheckbox = document.getElementById('selectAllPelanggan');
            const bulkDeleteBtn = document.getElementById('btnBulkDelete');
            const selectedCountEl = document.getElementById('selectedCount');

            // Update Select All Checkbox state (sync master with children)
        ...
        function submitBulkDelete() {
            ...
            inputsContainer.innerHTML = ''; // clear old ones
            ...
            confirmHapus(form, `${checkedCheckboxes.length} pelanggan terpilih`);
        }
    </script>
@endpush