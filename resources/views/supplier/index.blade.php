...
@section('content')
    ...
    <div class="supplier-layout">
                            ...
                            <td>
                                <div class="aksi-wrap">
                                    ...
                                    <form method="POST" action="{{ route('supplier.destroy', $s->id) }}"
                                        id="form-hapus-supplier-{{ $s->id }}" style="display:inline">
                                        @csrf @method('DELETE')
                                        <button type="button" class="btn-hapus" title="Hapus"
                                            onclick="confirmHapus(document.getElementById('form-hapus-supplier-{{ $s->id }}'), '{{ addslashes($s->nama_supplier) }}')">
                                            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                                stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
        ...
        <div class="card form-panel" id="formPanel">
            ...
            <form method="POST" id="formSupplier" action="{{ route('supplier.store') }}">
                @csrf
...
@push('scripts')
    <script>
        ...
        function openForm() {
            ...
            document.getElementById('formSupplier').action = '{{ route("supplier.store") }}';
        ...
        function editSupplier(id, nama, wa, alamat, kategori, deskripsi) {
            ...
            document.getElementById('formSupplier').action = '/supplier/' + id;
