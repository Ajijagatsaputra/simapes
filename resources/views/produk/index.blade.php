...
<style>
    ...
    .search-wrap input {
        ...
        font-size: .8rem; color: #1a2b4a; width: 160px; font-family: inherit;
    }
    ...
    .stok-ok    { background: #e8f8ee; color: #34c472; }
    .stok-low   { background: #fff3e6; color: #f5a54a; }
    .stok-empty { background: #fdeaea; color: #e05a5a; }
    ...
    .btn-edit:hover, .btn-hapus:hover { opacity: .85; transform: scale(1.08); }

    /* ── Pagination ── */
    .pagination-wrap { display: flex; align-items: center; justify-content: center; gap: 6px; margin-top: 18px; }
    .page-btn {
        width: 32px; height: 32px;
        border: 1px solid #dde8f8; border-radius: 8px;
    ...
    .form-panel .panel-title {
        font-size: .95rem; font-weight: 700; color: #1a2b4a;
    ...
    .btn-batal {
        padding: 8px 18px; background: #f0f4fb;
        border: 1px solid #dde8f8; border-radius: 9px;
        color: #5a7090; font-size: .82rem; font-weight: 600;
    ...
    /* ── Alert ── */
    .alert-success {
        background: #e8f8ee; border: 1px solid #b6e8cb;
        color: #217a44; border-radius: 10px;
...
@section('content')
    ...
    <div class="produk-layout">
                ...
                <tbody id="tableBody">
                    ...
                    <tr data-search="{{ strtolower($p->nama_produk . ' ' . $p->jenis_seragam) }}">
                        ...
                        <td style="max-width:240px;">
                            <span title="{{ $p->deskripsi }}">
                                {{ \Str::limit($p->deskripsi, 60) ?? '-' }}
                            </span>
                        </td>
                        <td class="center">
                            @php
                                $stokClass = $p->stok === 0 ? 'stok-empty' : ($p->stok <= 10 ? 'stok-low' : 'stok-ok');
                            @endphp
                        ...
                        <td>
                            <div class="aksi-wrap">
                                ...
                                <form method="POST" action="{{ route('produk.destroy', $p->id) }}"
                                      id="form-hapus-produk-{{ $p->id }}" style="display:inline">
                                    @csrf @method('DELETE')
                                    <button type="button" class="btn-hapus" title="Hapus"
                                        onclick="confirmHapus(document.getElementById('form-hapus-produk-{{ $p->id }}'), '{{ addslashes($p->nama_produk) }}')">
                                        <svg width="13" height="13" viewBox="0 0 24 24" fill="none"
                                             stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
        ...
        <div class="card form-panel" id="formPanel">
            ...
            <form method="POST" id="formProduk" action="{{ route('produk.store') }}">
                @csrf
                ...
                <div class="form-group">
                    ...
                    <input class="form-input" type="text" id="namaProduk" name="nama_produk"
                           placeholder="Cth: Celana Pramuka" required>
                </div>
...
<script>
    ...
    function editProduk(id, nama, jenis, harga, deskripsi, stok) {
        ...
        document.getElementById('formProduk').action = '/produk/' + id;
