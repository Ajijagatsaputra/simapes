...
@push('styles')
    @include('prediksi.partials.styles')
@endpush
...
@section('content')
    ...
    @include('prediksi.partials.form')

    @if(!$hasData)
        {{-- ── Tampilan Peringatan Jika Data Tidak Cukup ── --}}
        @include('prediksi.partials.warning')
    @else
        {{-- ── Stat Cards Grid (Tampil jika ada data) ── --}}
        @include('prediksi.partials.stats')

        {{-- ── Main Layout: Chart + Tabel + MRP ── --}}
        <div class="prediksi-layout">
            @include('prediksi.partials.chart_table')
            @include('prediksi.partials.mrp')
        </div>

        {{-- ── Rekomendasi Supplier ── --}}
        @include('prediksi.partials.supplier')
    @endif

@endsection

@push('scripts')
    @include('prediksi.partials.scripts')
@endpush